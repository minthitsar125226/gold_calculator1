...
if menu == "🏠 ပင်မ စာမျက်နှာ":
    ...
    if mode == "လက်စွပ် (Ring)":
        ...
        st.markdown(f"<div class='kanote-border'><h3>နံပါတ်: {r_no}</h3><p>Diameter: {details['mm']} mm</p></div>", unsafe_allow_html=True)
    elif mode == "လက်ကောက် (Bangle)":
        b_inch = st.number_input("အချင်း (လက်မ):", min_value=1, value=2)
        b_pe = st.number_input("အချင်း (ပဲ):", 0, 15, 0)
        c_inch, c_pe = logic.bangle_diameter_to_length(b_inch, b_pe)
        st.markdown(f"<div class='kanote-border'><h3>ပတ်လည်: {c_inch} လက်မ {c_pe} ပဲ</h3></div>", unsafe_allow_html=True)

elif menu == "📋 အထည်ယူ/အထည်အပ်":
    st.subheader("📋 အထည်ယူ/အထည်အပ် မှတ်တမ်း")
    ...
    with col1:
        ...
        user_carat = st.number_input("ကျောက်အလေးချိန် (Carat):", 0.0, step=0.01)
        price_per_carat = st.number_input(f"တစ် {gem_type} ဈေးနှုန်း:", 0, step=10000)
        g_kyat, g_pae, g_yway, g_point = st.number_input("ရွှေ(ကျပ်)",0), st.number_input("ရွှေ(ပဲ)",0,15), st.number_input("ရွှေ(ရွေး)",0,7), st.number_input("ရွှေ(Pt)",0.0)
        gold_price = st.number_input("ရွှေဈေး:", value=10000000)
    ...
    with col2:
        ...
        st.markdown(f"<div class='result-card'><h4>ကျောက်ဖိုး: {gem_cost:,.0f} ကျပ်</h4><p>ရွှေချိန်: {res['kyat']} ကျပ် {res['pae']} ပဲ</p></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='result-card'><h4>ရွှေဖိုး: {gold_cost:,.0f} ကျပ်</h4></div>", unsafe_allow_html=True)
        st.markdown(f"<div class='kanote-border'><h2>စုစုပေါင်း: {total_sum:,.0f} ကျပ်</h2></div>", unsafe_allow_html=True)
